...
import { ChevronDown, ExternalLink, Trash2, Clock, MessageSquarePlus, Paperclip, FileText, Upload } from "lucide-react";
      ...
      <AnimatePresence>
        {expanded && (
          <motion.div
            ...
            className="border-t border-border-subtle dark:border-border-subtle-dark px-4 py-3 bg-surface dark:bg-surface-dark"
          >
            <div className="space-y-2 max-h-48 overflow-y-auto mb-3">
              {app.activityLogs?.length ? (
                app.activityLogs.map((log) => (
                  <div key={log.id} className="text-sm">
                    <p className="text-ink dark:text-ink-dark">{log.content}</p>
                ...
                <p className="text-xs text-muted dark:text-muted-dark italic">No notes yet</p>
              )}
            </div>

            <div className="mb-3">
              <p className="text-xs font-medium text-muted dark:text-muted-dark mb-1.5">Attachments</p>
              {app.attachments?.length > 0 && (
                <div className="space-y-1 mb-2">
                  {app.attachments.map((file) => (
                    <div key={file.id} className="flex items-center justify-between text-sm bg-elevated dark:bg-elevated-dark rounded p-2">
                      <a
                        href={file.url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-2 text-accent hover:underline truncate"
                      >
            ...
            <form onSubmit={handleAddNote} className="flex gap-2">
              <input
                ...
                className="flex-1 text-sm p-2 rounded bg-elevated dark:bg-elevated-dark border border-border-subtle dark:border-border-subtle-dark focus:outline-none focus:ring-2 focus:ring-accent"
              />
              <button
                type="submit"
                disabled={submittingNote}
                className="text-accent hover:text-accent-hover disabled:opacity-50 p-2"
              >
