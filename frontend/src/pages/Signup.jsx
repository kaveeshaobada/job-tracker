...
function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.post("/auth/signup", { email: email.trim(), password });
      login(res.data.token, res.data.user);
      navigate("/");
    } catch (err) {
      console.error(err);
      ...
      } else if (err.response.status === 409 || err.response.data?.error === "Email already in use") {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center p-4">
      ...
      >
        ...
        <div>
          <label htmlFor="signup-email" className="block text-xs text-gray-300 mb-1 font-medium">
            Email Address
          </label>
          <input
            id="signup-email"
        ...
        <div>
          <label htmlFor="signup-password" className="block text-xs text-gray-300 mb-1 font-medium">
            Password
          </label>
          <input
            id="signup-password"
        ...
        <div>
          <label htmlFor="signup-confirm-password" className="block text-xs text-gray-300 mb-1 font-medium">
            Confirm Password
          </label>
          <input
          ...
          />
          {fieldErrors.confirmPassword && (
            <p className="text-red-400 text-xs mt-1 font-medium">{fieldErrors.confirmPassword}</p>
          )}
        </div>
